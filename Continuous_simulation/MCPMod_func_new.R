...
MCPMod_sim= function(N,d, Eff_mean,Eff_std,P_Tox,rho, 
    ...
    for (ii in 1:nsim) {
        ...
        Eff_models <- Mods(doses=d, placEff=0.2, maxEff=0.6,
                           linear = NULL, linlog = NULL, emax = 0.6,
                           exponential = c(1), logistic=c(1, 0.4),
                           fullMod = FALSE,
                           addArgs=list(off=1), direction = "increasing")
        
        Eff_dfe <- DoseFinding::MCPMod(dose,resp, data=Eff_data, models=Eff_models, alpha = 0.05, type = "normal", selModel = "maxT",
                                       pVal = TRUE, doseType = "TD", Delta = mu_low, alternative="one.sided")
        ...
        if(is.null(Eff_dfe$selMod)){
            ...
            fitted_mu<-unlist(predict(Eff_dfe, predType = "ls-mean", doseSeq = d)[Eff_dfe$selMod])
