...
ET_data <- function(r, sdev, Eff, n, Tox, score, gamma_index){
...
Eff_monitor=function(dat, n,post_tox, eff_cap, Cf, n_post,Tox_data){
    ...
    for (j in 1:length(n)) {
        ...
        if(num_T0[j]==0){
            ...
            Eff0_outcome[[j]]<-((OG_Eff0_outcome[[j]] - min(score))/
                                    (max(score) - min(score)))
        }
        ...
        if(num_T1[j]==0){
            ...
            Eff1_outcome[[j]]<-((OG_Eff1_outcome[[j]] - min(score))/
                                    (max(score) - min(score)))
        }
        ...
        unrestricted_post_mu0[,j]<-(unrestricted_post_mu0[,j]* (max(score) - min(score)) +
                                        min(score) )
        unrestricted_post_mu1[,j]<-(unrestricted_post_mu1[,j]* (max(score) - min(score)) +
                                        min(score) )
