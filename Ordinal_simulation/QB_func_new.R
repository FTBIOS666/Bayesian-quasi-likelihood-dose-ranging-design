...
QB_sim<-function(N1,ncohort,
    ...
    for (ii in 1:nsim){
        ...
        ob_data = ET_data(r=rho, sdev=Eff_std, Eff=Eff_mean, n=N_dose, Tox=P_Tox,
                          score, gamma_index)$sim_data
        ...
        while(k_flag<K){
            ...
            if(sum(pre_retain_dose[-1])==0 ){
                ...
                #generate new data
                #--------------------------------
                new_ob_data = ET_data(r=rho, sdev=Eff_std, Eff=Eff_mean, n=allo_res$npt, Tox=P_Tox,
                                      score, gamma_index)$sim_data
                for(i in 1:J){
