...
U_curve=Eff_ord_mean-4*Tox_curve
...
Eff_ord=sort(unique(as.vector(Eff_ord_mean[c(2,4,5,6,8,9),]))) 
...
library(readxl)
setting5=data.frame(read_excel("setting_U2.xlsx"))
...
dat=gamma_index[gamma_index[,1] %in% Eff_ord,]

write_xlsx(as.data.frame(dat),"dat.xlsx")
